...
Shader Shader::compileFromSource(std::string const & name,
    ...
    retShader.m_vertexShader = glCreateShader(GL_VERTEX_SHADER);
...
Shader Shader::compileFromFile(std::string const & name,
    ...
    return Shader::compileFromSource(name, vertexSource, fragmentSource);
}
...
bool Shader::set1f(std::string const & name, float v0){
    ...
    std::cout<<"Shader uniform location \""<<name<<"\" does not exist in "<<m_name<<std::endl;
    return false;
}
